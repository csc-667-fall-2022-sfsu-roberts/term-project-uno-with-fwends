...
const LOOUP_USER = "SELECT * FROM users WHERE email=${username}"; 

const REGISTER_USER = 
"INSERT INTO users(email, password) VALUES (${username}, ${password}) RETURNING id, email";

const FIND_USER = "SELECT * FROM users WHERE email=${username}"; 


const login = ({ username, password }) =>{
    return db.one(LOOUP_USER, { username } )
        .then(({ id, email, password: hash }) => Promise.all([
